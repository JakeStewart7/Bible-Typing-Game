...
export function createCampaignChunks(book: string): CampaignChunk[] {
  ...
  return chapters.flatMap((verseCount, chapterIndex) => {
    ...
    for (let verse = 1; verse <= verseCount; verse++) {
      chunks.push({
        id: `${book}:${chapterIndex + 1}:${verse}-${verse}`,
        book,
...
export function normalizeCampaignProgress(progress: CampaignProgress): CampaignProgress {
  const normalized: CampaignProgress = {};
  for (const [id, stars] of Object.entries(progress)) {
    const match = id.match(/^(.+):(\d+):(\d+)-(\d+)$/);
    if (!match) continue;
    const [, book, chapterText, startText, endText] = match;
    const chapter = Number(chapterText);
    const startVerse = Number(startText);
    const endVerse = Number(endText);
    const verseCount = VERSE_COUNTS[book]?.[chapter - 1] ?? 0;
    if (!verseCount || startVerse < 1 || endVerse < startVerse || endVerse > verseCount) continue;
    for (let verse = startVerse; verse <= endVerse; verse++) {
      const verseId = `${book}:${chapter}:${verse}-${verse}`;
      normalized[verseId] = Math.max(normalized[verseId] ?? 0, stars);
    }
  }
  return normalized;
}
...
export function completePassage(
  ...
  const completed = { ...normalizeCampaignProgress(progress) };
  for (let verse = passage.startVerse; verse <= passage.endVerse; verse++) {
    const id = `${passage.book}:${passage.chapter}:${verse}-${verse}`;
